...
export const fetchImages = async (keyword: string): Promise<DisplayImage[]> => {
  if (!GOOGLE_API_KEY || !GOOGLE_CX_ID) {
    console.error(
      "Google API Key or CX ID is not configured in environment variables."
    );
    // 실제 사용자에게는 좀 더 친절한 메시지를 보여주는 것이 좋습니다.
    // 예를 들어, Vercel 환경 변수 설정 방법을 안내하거나, .env.local 파일을 확인하라는 메시지 등
    throw new Error(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorCode =
        "code" in apiError ? (apiError as { code?: number }).code : undefined;
    ...
    if (data.items && data.items.length > 0) {
      return data.items
        ...
        .map((item: GoogleSearchImageItem) => ({
          id: item.link,
